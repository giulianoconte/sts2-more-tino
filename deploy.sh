...
if $RELEASE; then
    ...
    dotnet publish $SCRIPT_DIR/MoreTino/MoreTino.csproj --nologo -v quiet -c Release /p:DeployToMods=true /p:ModsPath=$DIST_DIR/
    ...
    if $INSTALL; then
        ...
        cp "$ARCHIVE" "$MODS_DIR/"
        rm -rf "$MODS_DIR/MoreTino"
        unzip -q "$MODS_DIR/$(basename "$ARCHIVE")" -d "$MODS_DIR"
    fi
else
    dotnet build $SCRIPT_DIR/MoreTino/MoreTino.csproj --nologo -v quiet /p:DeployToMods=true
fi
{ set +x; } 2>/dev/null
